...
RSpec.describe SessionsController, type: %i[api controller] do
  describe '#create' do
    ...
    it 'when login is successful' do
      post_request(params: { login_form: { email: 'test@test.com', password: '12345678', user_type: } }, path: :create)
      expect(session).to have_key(:auth_token)
      # TO-DO test the redirect
    end

    it 'when login is unsuccessful' do
      post_request(params: { login_form: { email: 'test@test.com', password: '123456789', user_type: } }, path: :create)
